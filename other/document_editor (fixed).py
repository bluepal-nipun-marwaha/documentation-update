...
Replaces text in Word documents while preserving formatting.
"""
...
def rephrase_text_with_formatting(original_text, original_formats, context_before="", context_after="", api_key=""):
    """
    Rephrases text using OpenAI with formatting awareness.
    
    Args:
        original_text: Original text to rephrase
        original_formats: List of formatting info from capture_detailed_formatting
        context_before: Text from line(s) before (for context)
        context_after: Text from line(s) after (for context)
        api_key: OpenAI API key
        
    Returns:
        List of dicts with 'text' and formatting for each segment
    """
    try:
        client = OpenAI(api_key=api_key)
        
        # Create formatting instruction
        formatting_desc = create_formatting_instruction(original_formats)
        ...
        estimated_tokens = len(original_text.split()) * 2
        max_tokens = max(200, min(estimated_tokens, 1000))
        
        prompt = f"""You are rephrasing text while preserving its formatting structure.
...
INSTRUCTIONS:
1. Rephrase the text to have similar meaning but different wording
2. Keep approximately the same length
3. Maintain the SAME formatting pattern (if original has bold label followed by normal text, your rephrase should too)
4. Return ONLY the rephrased text with markers: Use <BOLD>text</BOLD>, <ITALIC>text</ITALIC>, <UNDERLINE>text</UNDERLINE>
5. If original has no special formatting, just return plain rephrased text

Example input: "Key Strengths: Ready for production"
Formatting: Part 1: "Key Strengths:" [BOLD], Part 2: " Ready for production" [NORMAL]
Example output: <BOLD>Important Points:</BOLD> Prepared for deployment

YOUR REPHRASED TEXT:"""
        
        logger.info(f"Calling OpenAI API with formatting awareness...")
        logger.info(f"Formatting pattern: {len(original_formats)} parts")
        
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You rephrase text while preserving formatting patterns. You use <BOLD>, <ITALIC>, <UNDERLINE> tags to indicate formatting."},
        ...
        logger.info(f"LLM response: '{rephrased}'")
...
def main():
    ...
    PROCESS_ALL = False  # Change this to True to process everything
    ...
    try:
        ...
        for idx, item in enumerate(items_to_process, 1):
            ...
            try:
                ...
                # Rephrase with formatting awareness
                llm_response = rephrase_text_with_formatting(
                    item['text'], 
                    original_formats,
                ...
                # Parse LLM response
                formatted_segments = parse_formatted_response(llm_response, original_formats)
                
                # Apply formatting
                if formatted_segments is None:
                    # No tags found, use proportional distribution
                    logger.info("Using proportional distribution fallback")
                    replace_paragraph_text(item['object'], llm_response.replace('<BOLD>', '').replace('</BOLD>', '').replace('<ITALIC>', '').replace('</ITALIC>', '').replace('<UNDERLINE>', '').replace('</UNDERLINE>', ''), original_formats)
                else:
                    # Use LLM-generated segments
                    logger.info("Using LLM-generated formatting segments")
                    replace_paragraph_text(item['object'], formatted_segments, original_formats)
                
                logger.info(f"AFTER: '{item['object'].text[:80]}...'")
                
                # Verify replacement worked
                if item['text'] in item['object'].text:
                    logger.error("ERROR: Original text still present!")
                else:
                    logger.info("SUCCESS: Text replaced")
                    processed_count += 1
